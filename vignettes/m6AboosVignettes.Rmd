...
The package is available in GitHub and can be installed via the 
following code: 

```{r install, eval=FALSE}
install_github("ZarnackGroup/m6Aboost")
```

Load the package into an R session with
...
the relative signal strength. The function `getFeatures` allows to extract
these features. Please note the function `getFeatures` requires
...
uses the output object from the function `getFeatures`. In addition, the user 
